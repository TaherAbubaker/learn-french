...
const vocab = [
  ...
  },
  {
    ...
    en: "Nice to meet you",
    emoji: "🤝",
    ex: "Enchanté de faire votre connaissance.",
    cat: "greetings",
  },
  {
    fr: "Comment ça va?",
  ...
  },
  {
    ...
    en: "So-so",
    emoji: "🤷",
    ex: "Comme ci comme ça, pas terrible.",
    cat: "greetings",
  },
  {
    fr: "Un / Une",
  ...
  },
  {
    ...
    en: "The (m/f/plural)",
    emoji: "📌",
    ex: "Le livre, la table, les élèves.",
    cat: "pronouns",
  },
  {
    fr: "Un / Une / Des",
    en: "A / Some",
    emoji: "➕",
    ex: "Un homme, une femme, des enfants.",
    cat: "pronouns",
  },
  {
    fr: "S'il vous plaît",
  ...
  },
  {
    ...
    en: "Can you repeat?",
    emoji: "🔁",
    ex: "Pouvez-vous répéter, s'il vous plaît?",
    cat: "phrases",
  },
  {
    fr: "Où est...?",
  ...
  },
  {
    ...
    en: "How old are you?",
    emoji: "🎂",
    ex: "Quel âge avez-vous? J'ai 20 ans.",
    cat: "phrases",
  },
  {
    fr: "D'où venez-vous?",
  ...
  },
  {
    ...
    en: "Daughter / Girl",
    emoji: "👧",
    ex: "Leur fille est intelligente.",
    cat: "family",
  },
  {
    ...
    en: "Grandparents",
    emoji: "👴👵",
    ex: "Mes grands-parents sont âgés.",
    cat: "family",
  },
  {
    fr: "L'ami / L'amie",
  ...
  },
  {
    ...
    en: "The classroom",
    emoji: "🏫",
    ex: "La salle de classe est grande.",
    cat: "classroom",
  },
  {
    fr: "Le professeur / la prof",
    en: "The teacher",
    emoji: "🧑‍🏫",
    ex: "Le professeur explique.",
    cat: "classroom",
  },
  {
...
const frogCosmetics = [
  { emoji: "🐸", label: "plain frog", desc: "humble beginnings." },
  {
    ...
    hat: "🧣",
    label: "scarf frog",
    desc: "staying warm while studying 🧣",
  },
  {
    emoji: "🐸",
    hat: "👓",
    label: "glasses frog",
    desc: "intellectual era 👓",
  },
  { emoji: "🐸", hat: "🎩", label: "fancy frog", desc: "très chic 🎩" },
  {
    ...
    hat: "🪖",
    label: "warrior frog",
    desc: "in study battle mode 🪖",
  },
  { emoji: "🐸", hat: "👑", label: "crown frog", desc: "royalty. obviously." },
  {
    emoji: "🐸",
    hat: "🎓",
    label: "graduation frog",
    desc: "french degree loading... 🎓",
  },
...
function showLevelUp(level) {
  ...
  document.getElementById("reward-title").textContent =
    "🎉 level " + level + "!";
  document.getElementById("reward-cos-label").textContent =
    cos.label + " — " + cos.desc;
...
function renderNotesList(filter) {
  ...
  if (!list.length) {
    wrap.innerHTML =
      '<div class="notes-empty">no notes yet 🐸<br>tap "+ new note" to start!</div>';
    return;
  }
  wrap.innerHTML = list
    .map((n) => {
      ...
      const date = new Date(n.updated).toLocaleDateString("en-US", {
        month: "short",
        day: "numeric",
      });
      return (
        '<div class="note-item ' +
        (n.id === currentNoteId ? "active" : "") +
        '" onclick="selectNote(\'' +
        n.id +
        "')\">" +
...
function detectWords(text) {
  ...
  wrap.innerHTML = Object.entries(found)
    .map(
      ([w, t]) =>
        '<div class="word-pill">' +
        escapeHtml(w) +
...
function searchDict(q) {
  ...
  if (!q.trim()) {
    res.innerHTML =
      '<p style="font-size:0.85rem;color:var(--muted);text-align:center;padding:2rem 0">type a word to search 🐸</p>';
    return;
  }
  const matches = vocab.filter(
    (v) =>
  ...
  if (!matches.length) {
    res.innerHTML =
      '<p style="font-size:0.85rem;color:var(--muted);text-align:center;padding:2rem 0">no results try another word 🐸</p>';
    return;
  }
  res.innerHTML = matches
    .map(
      (m) =>
        ...
        m.ex +
        '"</div>' +
        '<button class="dict-speak" onclick="speak(\'' +
        m.fr.replace(/'/g, "\'") +
        "')\">🔊 pronounce</button>" +
        "</div>",
    )
...
function nextQuiz() {
  ...
  if (mode === "conjugate") {
    ...
    document.getElementById("quiz-q").textContent =
      c.pronoun + " ___ (" + c.verb + ")";
    const opts = [c.form];
    const pool = conjugations
      .filter((x) => x.verb === c.verb && x.form !== c.form)
      .map((x) => x.form);
    ...
    document.getElementById("quiz-opts").innerHTML = opts
      .map(
        (o) =>
          '<button class="quiz-opt" onclick="checkAnswer(this,' +
          (o === c.form) +
          ",'" +
          c.form +
          "')\">" +
    ...
    document.getElementById("quiz-type").textContent = toEn
      ? "translate to english"
      : "translate to french";
    document.getElementById("quiz-q").textContent = toEn
      ? correct.fr
      : correct.en;
    const opts = [correct];
    while (opts.length < 4) {
    ...
    document.getElementById("quiz-opts").innerHTML = opts
      .map((o) => {
        ...
        return (
          ...
          isRight +
          ",'" +
          (toEn ? correct.en : correct.fr).replace(/'/g, "\'") +
          "')\">" +
          label +
          "</button>"
        );
...
function checkAnswer(btn, correct, rightAnswer) {
  ...
  if (correct) {
    ...
    const msgs = [
      "parfait! 🌟",
      "très bien! 🐸",
      "correct! wanna bet you ace this? 😏",
      "oui oui! 🎉",
    ];
    ...
    const fb = document.getElementById("quiz-feedback");
    fb.textContent =
      "not quite the answer is: " + rightAnswer + " 🐸 you got it next time!";
...
function renderMessages() {
  ...
  wrap.innerHTML = unlockMessages
    .map((m) => {
      const unlocked =
        unlockedMessages.includes(m.level) || currentLevel >= m.level;
      if (unlocked) {
        return (
          '<div class="msg-card unlocked' +
          (m.isLast ? " msg-last" : "") +
          '">' +
          '<div class="msg-card-header">' +
          '<span class="msg-level-badge">' +
          m.title +
          "</span>" +
          "</div>" +
          '<div class="msg-body">' +
          ...
          "</div>" +
          (m.hasPhoto
            ? `<img src="${m.photo}" class="msg-photo" alt="unlocked photo">`
            : "") +
          "</div>"
        );
      } else {
        return (
          '<div class="msg-card locked">' +
          '<div class="msg-lock-icon">🔒</div>' +
          '<div class="msg-lock-text">reach level ' +
          m.level +
          " to unlock</div>" +
          '<div class="msg-lock-xp">need ' +
          (m.level - currentLevel) * xpPerLevel +
          " more XP</div>" +
          "</div>"
        );
      }
    ...
    pomodoroInterval = setInterval(() => {
      ...
      if (pomodoroTime <= 0) {
        ...
        if (!isBreak) {
          ...
          showRoomBubble(
            "YAYYY break time!! tu l'as fait! 🎉 go drink water pretty eyes",
          );
          ...
          setFrogMood("idle");
          showRoomBubble(
            "okay back to work silly 🐸 wanna bet u ace this session too?",
          );
...
function setFrogMood(mood) {
  ...
  setTimeout(() => {
    roomFrog.style.animation = "";
    roomFrog.classList.remove("frog-study", "frog-break", "frog-celebrate");
    roomFrog.classList.add("frog-" + mood);
  }, 10);
}
...
async function sendChat() {
  ...
  if (getChatUsage() >= CHAT_DAILY_LIMIT) {
    ...
    limitMsg.textContent =
      "you've hit today's chat limit (" +
      CHAT_DAILY_LIMIT +
      " messages) — come back tomorrow, or keep studying with flashcards and quizzes in the meantime! 🐸";
  ...
  try {
    ...
    if (!session) {
      ...
      botMsg.textContent = "please log in to chat with me 🐸";
    ...
    const reply =
      data.reply || "sorry, something went wrong — try again in a moment 🐸";
    typing.remove();
    const botMsg = document.createElement("div");
    botMsg.className = "msg bot";
